...
namespace RenownedGames.ExLibEditor.Windows
{
    public class ExSearchWindow : ScriptableObject, ISearchWindowProvider, IExSearchWindow, IExSearchWindowEntry
    {
        ...
        public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
        {
            ...
            const string ROOT_GROUP_NAME = "Root";
            SearchTreeGroupEntry rootGroup = new SearchTreeGroupEntry(new GUIContent(ROOT_GROUP_NAME), -1);
            Dictionary<SearchTreeGroupEntry, List<SearchTreeEntry>> groups = new Dictionary<SearchTreeGroupEntry, List<SearchTreeEntry>>
            {
                { rootGroup, new List<SearchTreeEntry>() }
            };
            ...
            for (int i = 0; i < entries.Count; i++)
            {
                ...
                {
                    string path = paths[j];
                    groupBuilder.Append(path);
                    if (!groups.Any(t => t.Key.name == path))
                    {
                        groups.Add(new SearchTreeGroupEntry(new GUIContent(path), j + 1), new List<SearchTreeEntry>());
                    }
                    groupBuilder.Append('/');
                }
                ...
                SearchTreeGroupEntry groupEntry = rootGroup;
                if (length > 0)
                {
                    string section = paths[length - 1];
                    foreach (SearchTreeGroupEntry item in groups.Keys)
                    {
                ...
                if(groups.TryGetValue(groupEntry, out List<SearchTreeEntry> value))
                {
                    value.Add(searchTreeEntry);
                }
            }
            ...
            {
                if (pair.Key.name != ROOT_GROUP_NAME && pair.Key.level > -1)
                {
                    treeEntries.Add(pair.Key);
                }
