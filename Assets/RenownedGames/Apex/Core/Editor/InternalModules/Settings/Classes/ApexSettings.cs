...
namespace RenownedGames.ApexEditor
{
    ...
    public sealed class ApexSettings : ScriptableSingleton<ApexSettings>
    {
        ...
        [SerializeField]
        [OnValueChanged(nameof(IncludeAllAssembliesChanged))]
        private bool includeAllAssemblies = true;

        [SerializeField]
        [DisableIf(nameof(includeAllAssemblies))]
        private bool includeDefaultAssemblies = true;

        [SerializeField]
        [Array]
        [HideIf(nameof(includeAllAssemblies))]
        ...
        private void ApplySettings()
        {
            ...
            if (assemblyScopes != null)
            {
                for (int i = 0; i < assemblyScopes.Length; i++)
                {
                    ...
                    {
                        Debug.LogWarning($"<b>Apex:</b> An attempt to add assembly scope with empty or white space name. Invalid scope: <color=red>index {i+1}</color>");
                        continue;
                    }

                    if (!ApexUtility.AssemblyScopes.Add(assemblyScope))
                    {
                        Debug.LogWarning($"<b>Apex:</b> An attempt to add two identical assembly scope. Duplicate scopes: <color=red>{assemblyScope.GetName()}</color>");
                    }
                }
            }
            ...
            if(exceptTypes != null)
            {
                for (int i = 0; i < exceptTypes.Length; i++)
                {
                    ...
                    {
                        Debug.LogWarning($"<b>Apex:</b> An attempt to add type with empty or white space name. Invalid type: <color=red>index {i + 1}</color>");
                        continue;
                    }

                    if (!ApexUtility.ExceptTypes.Add(exceptType))
                    {
                        Debug.LogWarning($"<b>Apex:</b> An attempt to add two identical types for an exception. Duplicate types: <color=red>{exceptType.GetName()}</color>");
                    }
                }
            }
        }
        ...
        public bool IncludeAllAssembly()
        {
            return includeAllAssemblies;
        }

        public void IncludeAllAssembly(bool value)
        {
            includeAllAssemblies = value;
        }

        public bool IncludeDefaultAssembly()
        {
            return includeDefaultAssemblies;
        }

        public void IncludeDefaultAssembly(bool value)
        {
            includeDefaultAssemblies = value;
        }
        ...
        public AssemblyScope[] GetAssemblyScopes()
        {
            return assemblyScopes;
        }

        public void SetAssemblyScopes(AssemblyScope[] value)
        {
            assemblyScopes = value;
        }
        #endregion
    }
}